...
def pre_label_program_nodes(voxel_graph, local_graph,
    ...
    dy = maxY - minY
    dx = maxX - minX
    if abs(dy) < 1e-9: dy = 1.0
    if abs(dx) < 1e-9: dx = 1.0

    # (2) 각 프로그램 노드 정규화된 위치 계산
    for nd in local_graph["node"]:
        f = nd["floor"]
        zc = nd["center"][0]
        raw_y = nd["center"][1]
        ...
        # (y, x) 정규화 → base_length/base_width 범위
        norm_y = (raw_y - minY) / dy * base_length
        norm_x = (raw_x - minX) / dx * base_width

        # (3) 복셀 중에서 해당 위치에 해당하는 voxel 찾기
        found = False
        for vx in voxel_nodes:
            ...
            if (z0 <= zc <= z1) and (y0 <= norm_y <= y1) and (x0 <= norm_x <= x1):
                ...
                break
        if not found:
            print(f"[WARN] Node {nd} not found in voxel after normalization.")
