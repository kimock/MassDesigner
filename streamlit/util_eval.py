...
def postprocess_core_labels(out, graph, threshold: float = 0.2, lobby_threshold: float = 0.8):
    """
    · CORE_IDS(1,2,3,5)을 동일 그룹으로 보고,
      - 같은 수직 칼럼(= voxel_projection_cluster) 안에서
      - (CORE 비율 ≥ threshold) 이면 전부 코어로 통일
      - (CORE 비율 < threshold) 이면 코어 라벨만 제거
         → 단, 로비 비율이 충분히 높으면 전체를 로비로 통일
    """
    ...
    core_ratio = core_cnt.float() / (tot_cnt.clamp_min(1))

    for col_id in range(num_cols):
        if core_cnt[col_id] == 0:
            continue  # 코어 없는 칼럼은 무시
